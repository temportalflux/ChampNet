...
public:
	...
	void setSteering(SteeringType type, Vector2D targetLoc = ZERO_VECTOR2D, UnitID targetUnitID = INVALID_UNIT_ID);
	void setSteering(SteeringData& data);
	void setSteeringTarget(Vector2D targetLoc = ZERO_VECTOR2D, UnitID targetUnitID = INVALID_UNIT_ID);
	void onDataChanged();
...
private:
	...
	PhysicsComponent* mpComponentPhysics = NULL;
	SteeringComponent* mpComponentSteering = NULL;
	Sprite mSprite;
	float mMaxAcc;
	float mMaxSpeed;
	float mMaxRotAcc;
